...
export async function resolvePhaseForDate(deviceId: string, observedAt: string): Promise<string | null> {
  try {
...
export async function getActivePhase(deviceId: string): Promise<ColonyPhase | null> {
  try {
    const { data, error } = await supabase
      .from('colony_phases')
      .select('*')
